...
def protease(seq, miscuts=2, rule="[KR]", **kwargs):
    nomiscuts = parser.cleave(seq, rule=rule, missed_cleavages=0)
    all_seqs = parser.cleave(seq, rule=rule, missed_cleavages=miscuts)

    # without M
    nomiscuts_without_m = parser.cleave(
        seq[1:], rule=rule, missed_cleavages=0, min_length=7
    )
    all_seqs_without_m = parser.cleave(
        seq[1:], rule=rule, missed_cleavages=miscuts, min_length=7
    )
